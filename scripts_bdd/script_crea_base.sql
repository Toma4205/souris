...
CREATE TABLE `ami` (`id_coach` MEDIUMINT UNSIGNED NOT NULL , `id_coach_ami` MEDIUMINT UNSIGNED NOT NULL , `date_demande` DATETIME NOT NULL , `date_ajout` DATETIME NULL , `date_refus` DATETIME NULL , PRIMARY KEY (`id_coach`, `id_coach_ami`)) ENGINE = InnoDB;
...
ALTER TABLE `ami` ADD FOREIGN KEY (`id_coach`) REFERENCES `coach`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `ami` ADD FOREIGN KEY (`id_coach_ami`) REFERENCES `coach`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
